...
const HostFacials = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      flex: 0.5,
      renderCell: (params) => {
        if (params.value === "P" || params.value === "A") {
          return "정상"
        } else {
          return "삭제"
        }
  ...
  const getFacialList = async () => {
    ...
    const body = {
      ...
      Mode: "GET_HOST_USER_FACIAL_LIST",
    }
    const headers = {
  ...
  return (
    <Box m="1.5rem 2.5rem">
      <Header title="얼굴 등록" subtitle="얼굴 등록 리스트" />
      <Box
        height="75vh"
        width="99%"
        sx={{
      ...
      >
        ...
        <DataGrid
          getRowId={(row) => row.user_facial_oid}
          rows={rows || []}
          columns={columns}
